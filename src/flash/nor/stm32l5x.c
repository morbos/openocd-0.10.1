...
static int stm32l5_write_options(struct flash_bank *bank)
{
	...
	int retval = stm32l5_unlock_option_reg(target);
	if (retval != ERROR_OK)
		return retval;
...
static int get_stm32l5_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	switch (device_id) {
	case 0x415:
		device_str = "STM32L5xx";
		break;

	case 0x435:
		device_str = "STM32L53x";
		break;

	case 0x461:
		device_str = "STM32L59x";
		break;
	case 0x470:
		device_str = "STM32L5Rx";
		break;
	case 0x495:
...
COMMAND_HANDLER(stm32l5_handle_mass_erase_command)
{
	...
	if (CMD_ARGC < 1) {
		command_print(CMD_CTX, "stm32x mass_erase <STM32L5 bank>");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
